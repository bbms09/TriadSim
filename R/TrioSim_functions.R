...
#' @export
#'
#' @examples tar.snp <- c(1084, 2044, 3285, 4016, 5117 ,6067, 7077, 8187)
#' m.file <- paste(system.file(package = "TrioSim"),'/extdata/pop1_4chr_mom',sep='')
#' f.file <- paste(system.file(package = "TrioSim"),'/extdata/pop1_4chr_dad',sep='')
...
#'
#' @examples 
#' tar.snp <- c(1084, 2044, 3285, 4016, 5117 ,6067, 7077, 8187)
...
#' should be 1+ number_of_risk_pathway. The first number gives the disease prevalence in the
...
fit.risk.model.par <- function(n.ped,brks,target.snp,fam.pos, mom.tar,dad.tar, kid.tar, pathways, betas.e0, e.fr=NA, betas.e,pop1.frac=NA,rate.beta= NA,is.case=TRUE, qtl=FALSE,out.put.file=NA,no_cores=NA) {
  ...
  sim.fam <- function(i) {
    ...
    while (aff==0) {
      ...
      if (!is.na(e.fr)) {betas.e <- betas.e.org; betas.e[1] <- betas.e[1] + (pop==2)*rate.beta}
      e.fr <- e.fr.org[pop]
      }
      n.gwas <- nrow(mom.tar)
      n.seg <- ncol(brks)-1
  ...
  e.vec <- sim.all[,2]
  pop.vec <- sim.all[,3]
  if (!is.na(e.fr)) if (is.na(out.put.file)) write(e.vec,ncolumns=1,file='exposure.txt') else write(e.vec,ncolumns=1,file=paste(out.put.file,'exp.txt'))
  if (two.pop)  if (is.na(out.put.file)) write(pop.vec,ncolumns=1,file='population.txt') else write(pop.vec,ncolumns=1,file=paste(out.put.file,'pop.txt'))
  if (qtl)  if (is.na(out.put.file)) write(pheno,ncolumns=1,file='phenotype.txt') else write(pop.vec,ncolumns=1,file=paste(out.put.file,'pheno.txt'))
  
  write(t(sel.fam.all),ncolumns=ncol(sel.fam.all),file='sel.fam.all')
...
#' \dontrun{
#' glue.chr.segment.par(c(m.file,f.file,k.file),'trio', breaks,sel.fam,
...
#' with the risk of unexposed individuals who carry no copies of the pathways as a reference.
...
#' @param qtl is a boolean variable denoting whether a quantitative trait (qtl=TRUE) or a binary trait (qtl=FALSE) is to be simulated. For a binary trait only affected 
#' families will be kept. The default value is qtl=FALSE.t6
#' @param same.brk is an indicator variable to denote whethere the same set of breaking points will be used for all simulated triads
#' @param flip is boolean indicating whether the mother's and the father's genotypes will be swapped to wipe out potential maternal effects in the orignal data
#'
...
#' data for one chromosome for all trios. Genotypes of the mothers, fathers, and children are stacked on top of each other. The first third of the rows are genotypes of the mothers'.
#' The seond third are those of the fathers' and the last third are the children's. 
...
#' TrioSim(input.plink.file, 'triad', fr.desire=0.05,pathways=list(1:4,5:8),n.ped=1000, 
#'        N.brk=3, target.snp=NA,P0=0.001,is.OR=FALSE,risk.exposure= 1,
#'        risk.pathway.unexposed=c(1.5, 2), risk.pathway.exposed=c(1.5, 2), is.case=TRUE, e.fr=NA, 
#'        pop1.frac=NA, rate.beta=0, rcmb.rate, no_cores=1)

### call glue.chr.segment.par to parallel by pedigree ###
TrioSim <- function(input.plink.file, out.put.file, fr.desire,pathways,n.ped, N.brk, target.snp=NA,P0, is.OR,
         risk.exposure, risk.pathway.unexposed, risk.pathway.exposed, is.case=TRUE, e.fr=NA, pop1.frac=NA, rate.beta,
...
### converting RR to OR
  ...
  if (!is.OR) { 
    ...
    if (!is.na(e.fr)) betas.e1 <- c(RR2OR(P0*risk.exposure,P0), RR2OR(risk.pathway.exposed,P0))  else betas.e1 <- NA
  } else {
    betas.e0 <- c(log(P0), log(risk.pathway.unexposed))
    if (!is.na(e.fr)) betas.e1 <- c(log(P0) +log(risk.exposure), log(risk.pathway.exposed)) else betas.e1 <- NA
  }
